<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Source+Code+Pro&size=34&pause=1000&color=ffffff&center=true&vCenter=true&width=600&height=50&lines=Hi+There!+I'm+Mushfiqul+Islam;Full+Stack+Web+Developer;Laravel+%7C+Vue.js+%7C+Tailwind;Clean+Code+%7C+Clean+UI" alt="Typing SVG" />
</div>
<p align="center">
  <img src="https://komarev.com/ghpvc/?username=mushfiqul21&label=Profile+Views&color=2ec4b6&style=flat" alt="profile views" />
  <img src="https://img.shields.io/github/followers/mushfiqul21?label=Followers&style=flat&color=2ec4b6" alt="followers" />
</p>

---

## 👨‍💻 About Me

```php
<?php

class Mushfiqul extends WebDeveloper
{
    public string $name     = "Mushfiqul Islam";
    public string $role     = "Web Developer";
    public string $location = "Bangladesh 🇧🇩";
    public string $experience = "1.5+ Years Professional";

    public array $currentlyDoing = [
        "💼 Working as a Laravel Developer",
        "⚛️  Learning React & building SaaS products",
        "🎨 Crafting clean UI with Tailwind CSS",
        "🚀 Exploring full-stack web development",
    ];

    public array $interests = [
        "SaaS Products",
        "Open Source",
        "Clean Code",
        "UI/UX Design",
    ];

    public string $funFact = "I turn coffee into Laravel code ☕";
}
```

---

## 🛠️ Tech Stack & Skills

#### 🔧 Backend
<p>
  <img src="https://skillicons.dev/icons?i=php" height="40" title="PHP"/>
  <img src="https://skillicons.dev/icons?i=laravel" height="40" title="Laravel"/>
  <img src="https://skillicons.dev/icons?i=mysql" height="40" title="MySQL"/>
</p>

#### ⚛️ Frontend
<p>
  <img src="https://skillicons.dev/icons?i=vue" height="40" title="Vue"/>
  <img src="https://skillicons.dev/icons?i=react" height="40" title="React"/>
  <img src="https://skillicons.dev/icons?i=js" height="40" title="JavaScript"/>
  <img src="https://skillicons.dev/icons?i=jquery" height="40" title="jQuery"/>
  <img src="https://skillicons.dev/icons?i=html" height="40" title="HTML5"/>
  <img src="https://skillicons.dev/icons?i=css" height="40" title="CSS3"/>
</p>

#### 🎨 Styling
<p>
  <img src="https://skillicons.dev/icons?i=tailwind" height="40" title="Tailwind CSS"/>
  <img src="https://skillicons.dev/icons?i=bootstrap" height="40" title="Bootstrap"/>
</p>

#### 🌐 CMS & Tools
<p>
  <img src="https://skillicons.dev/icons?i=wordpress" height="40" title="WordPress"/>
  <img src="https://skillicons.dev/icons?i=git" height="40" title="Git"/>
  <img src="https://skillicons.dev/icons?i=github" height="40" title="GitHub"/>
  <img src="https://skillicons.dev/icons?i=vscode" height="40" title="VS Code"/>
  <img src="https://skillicons.dev/icons?i=postman" height="40" title="Postman"/>
</p>

---

## 🚀 What I Build

| Type | Technologies |
|---|---|
| 🌐 Web Apps | Laravel, PHP, MySQL, REST API |
| ⚛️ Frontend | Vue.js, React, JavaScript, jQuery |
| 🎨 UI/UX | Tailwind CSS, Bootstrap, HTML, CSS |
| 🛒 CMS | WordPress, WooCommerce |
| 🔌 API | RESTful APIs, Laravel Sanctum |

---

## 💼 Experience

```
🏢 Web Developer
   └── 1.5+ Years Professional Experience
   └── Laravel Backend Development
   └── REST API Design & Integration
   └── Frontend Development
   └── Database Design (MySQL)
```

---

## 🌐 Connect With Me

<p align="center">
  <a href="https://linkedin.com/in/mushfiqul-islam-m35822">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
  </a>
  &nbsp;
  <a href="https://github.com/mushfiqul21">
    <img src="https://img.shields.io/badge/GitHub-1a2e2d?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
  </a>
  &nbsp;
  <a href="mailto:mushfiqulislam21@email.com">
    <img src="https://img.shields.io/badge/Email-2ec4b6?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
  </a>
  &nbsp;
  <a href="https://mushfiqulislam.dev](https://mushfiqul21.github.io/portfolio/">
    <img src="https://img.shields.io/badge/Portfolio-1fa89b?style=for-the-badge&logo=firefox&logoColor=white" alt="Portfolio"/>
  </a>
</p>

---

<p align="center">
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=light&quote=First%2C%20solve%20the%20problem.%20Then%2C%20write%20the%20code.&author=John%20Johnson"/>
</p>

<p align="center">
  <i>"Clean code always looks like it was written by someone who cares."</i>
</p>

